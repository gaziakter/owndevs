...
function owndevs_setup() {
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'owndevs' ),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'owndevs_custom_background_args',
			array(
 ...
 */
function owndevs_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'owndevs' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'owndevs' ),
...
add_action( 'widgets_init', 'owndevs_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function owndevs_scripts() {
	wp_enqueue_style( 'owndevs-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'owndevs-style', 'rtl', 'replace' );

	wp_enqueue_script( 'owndevs-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
